...
This tells us that for a smooth closed geodesic to exist, the total Gaussian curvature of the area it encloses must equal $$2\pi$$ (the "angle" of a full circle). This is why you can have a closed geodesic on a sphere ($$K > 0$$), but you can never have a simple closed geodesic on a flat plane ($$K=0$$) or a saddle ($$K<0$$)—the "curvature budget" doesn't add up to $$2\pi$$.
...
- Along the edge: Because you are on a geodesic, you never turn your "steering wheel." The spear stays parallel to your path.
- At the corner: You stop and turn your body by an exterior angle ($$\alpha_i$$). You do not turn the spear; it still points where it was pointing.
- Back at the start: When you complete the loop, you compare the spear's current direction to its starting direction. They won't match. This net rotation of the vector after the full trip, the "error" in direction, is called the __Holonomy ($$\Delta \theta$$)__.
