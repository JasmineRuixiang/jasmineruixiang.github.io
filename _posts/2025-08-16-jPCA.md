...
In this study, the authors are particularly interested in extracting potentially pure rotational dynamics. The key observation that Churchland and colleagues offered here is that every linear transformation $$M$$ (thus the linear dynamics of $$\dot{X}$$) can be decomposed into two components:
