...
As the authors argued, the data covariance matrix $$\Sigma$$ captures __an ellipsoid which best fits the data__. Specifically, the data covariance matrix $$\Sigma$$ (already demeaned) encodes the spread and correlation of data along different directions. The corresponding eigenvectors point at the principal directions of variation, while the eigenvalues indicate how much the data varies along those directions. 

but with no temporal information, whereas $$M^*$$ characterizes a linear dynamical system which best fits the data $$X$$. 
