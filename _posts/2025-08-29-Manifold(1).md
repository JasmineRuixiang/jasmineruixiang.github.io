...
Another slightly different counterexample for Hausdorffness is not on separating some point, but glueing spaces together: Consider two copies of Euclidean plane: $$\mathbb{R}^2_1, \mathbb{R}^2_2$$, and glue them on the right half plane $$H = \{(x, y) \in \mathbb{R}^2 | x \geq 0 \}$$. Equivalently, $$X = \mathbb{R}^2_1 \sqcup \mathbb{R}^2_2 / \sim$$, where $$(x_1, y_1) \sim (x_2, y_2)$$ iff $$x \geq 0$$. Obviously, $$X$$ is not Hausdorff since there's not disjoint neighbourhood around $$(0, 0)_1$$ and $$(0, 0)_2$$.
